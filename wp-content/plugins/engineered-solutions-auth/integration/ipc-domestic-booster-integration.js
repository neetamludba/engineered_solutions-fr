/**
 * IPC Domestic Booster Sizing Integration
 * This code should be added to IPC/domestic_booster_sizing.html
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const checkESA = setInterval(() => {
        if (typeof window.esaAuth !== 'undefined') {
            ...
            initializeIPCDomesticBoosterIntegration();
        }
    }, 100);
});

function initializeIPCDomesticBoosterIntegration() {
    ...
    if (typeof window.updateSummaryPage !== 'undefined') {
        ...
        window.updateSummaryPage = function(type, modelId) {
            ...
            saveIPCDomesticBoosterEstimate(type, modelId);
    ...
    // Initialize the page
    initializeIPCDomesticBoosterPage();

    // React to auth state changes without requiring a page refresh
    document.addEventListener('esaAuthState', handleIPCDomesticBoosterAuthState);
}

function handleIPCDomesticBoosterAuthState(event) {
    const state = event ? event.detail : null;
    refreshIPCDomesticBoosterView(state);
}

function refreshIPCDomesticBoosterView(state) {
...
function saveIPCDomesticBoosterEstimate(type, modelId) {
    // Collect form data
    const formData = collectIPCDomesticBoosterFormData();
    
    // Save estimate request
    if (window.esaAuth) {
        window.esaAuth.saveEstimateRequest('ipc_domestic_booster', modelId, formData);
    }
}

function collectIPCDomesticBoosterFormData() {
    const formData = {};
...
async function checkAndUpdateApprovalStatus() {
    try {
        ...
        console.error('ESA IPC Domestic Booster: Error checking approval status:', error);
    }
}
    ...
    const checkFunction = setInterval(() => {
        if (typeof window[funcName] === 'function') {
            ...
            window[funcName] = async function(...args) {
                console.log(`ESA IPC Domestic Booster: Intercepted ${funcName}`);
                ...
                if (window.esaAuth && window.esaAuth.isLoggedIn) {
                    console.log('ESA IPC Domestic Booster: Verifying approval status...');
                    try {
                        await checkAndUpdateApprovalStatus();
                    } catch (err) {
                        console.error('ESA IPC Domestic Booster: Error in approval check:', err);
                    }
                }
...
function initializeIPCDomesticBoosterPage() {
