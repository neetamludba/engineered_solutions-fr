...
async function checkAndUpdateApprovalStatus() {
    try {
        ...
        console.error('ESA MHC Rainwater: Error checking approval status:', error);
    }
}
    ...
    const checkFunction = setInterval(() => {
        if (typeof window[funcName] === 'function') {
            ...
            window[funcName] = async function(...args) {
                console.log(`ESA MHC Rainwater: Intercepted ${funcName}`);
                ...
                if (window.esaAuth && window.esaAuth.isLoggedIn) {
                    console.log('ESA MHC Rainwater: Verifying approval status...');
                    try {
                        await checkAndUpdateApprovalStatus();
                    } catch (err) {
                        console.error('ESA MHC Rainwater: Error in approval check:', err);
                    }
                }
