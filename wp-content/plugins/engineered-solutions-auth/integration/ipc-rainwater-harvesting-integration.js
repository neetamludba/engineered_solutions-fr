/**
 * IPC Rainwater Harvesting Sizing Integration
 * This code should be added to IPC/rainwater_harvesting_sizing.html
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const checkESA = setInterval(() => {
        if (typeof window.esaAuth !== 'undefined') {
            ...
            initializeIPCRainwaterHarvestingIntegration();
        }
    }, 100);
});

function initializeIPCRainwaterHarvestingIntegration() {
    ...
    if (typeof window.updateSummaryPage !== 'undefined') {
        ...
        window.updateSummaryPage = function(type, modelId) {
            ...
            saveIPCRainwaterHarvestingEstimate(type, modelId);
    ...
    setupPumpFunctionOverride('dayTankPumpType');
    
    // Initialize the page
    initializeIPCRainwaterHarvestingPage();

    // React to auth state changes across open applications
    document.addEventListener('esaAuthState', handleIPCRainwaterAuthState);
}

function handleIPCRainwaterAuthState(event) {
    const state = event ? event.detail : null;
    refreshIPCRainwaterView(state);
}

function refreshIPCRainwaterView(state) {
...
function saveIPCRainwaterHarvestingEstimate(type, modelId) {
    // Collect form data
    const formData = collectIPCRainwaterHarvestingFormData();
    
    // Save estimate request
    if (window.esaAuth) {
        window.esaAuth.saveEstimateRequest('ipc_rainwater_harvesting', modelId, formData);
    }
}

function collectIPCRainwaterHarvestingFormData() {
    const formData = {};
...
async function checkAndUpdateApprovalStatus() {
    try {
        ...
        console.error('ESA IPC Rainwater: Error checking approval status:', error);
    }
}
    ...
    const checkFunction = setInterval(() => {
        if (typeof window[funcName] === 'function') {
            ...
            window[funcName] = async function(...args) {
                console.log(`ESA IPC Rainwater: Intercepted ${funcName}`);
                ...
                if (window.esaAuth && window.esaAuth.isLoggedIn) {
                    console.log('ESA IPC Rainwater: Verifying approval status...');
                    try {
                        await checkAndUpdateApprovalStatus();
                    } catch (err) {
                        console.error('ESA IPC Rainwater: Error in approval check:', err);
                    }
                }
...
function initializeIPCRainwaterHarvestingPage() {
